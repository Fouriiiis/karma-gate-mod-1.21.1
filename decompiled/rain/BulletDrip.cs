...
public class BulletDrip : UpdatableAndDeletable, IDrawable
{
	...
	private void Strike()
	{
		...
		{
			pos.y = room.FloatWaterLevel(pos) - 30f;
			room.waterObject.WaterfallHitSurface(pos, 0f, 1f);
			if (room.game.rippleFade == 1f)
			{
				room.PlaySound(SoundID.Small_Object_Into_Water_Fast, pos);
			}
		}
		else if (room.game.rippleFade == 1f)
		{
			room.PlaySound(SoundID.Bullet_Drip_Strike, pos);
		}
		if (collisionResult.chunk != null && (!ModManager.Watcher || !room.game.IsStorySession || collisionResult.chunk.owner.abstractPhysicalObject.rippleLayer == 0))
		{
