...
public class GlobalRain
{
	public class DeathRain
	{
		...
		public void NextDeathRainMode()
		{
			...
			else if (ModManager.MSC && globalRain.game.IsStorySession && globalRain.game.GetStorySession.saveState != null && globalRain.game.StoryCharacter == MoreSlugcatsEnums.SlugcatStatsName.Rivulet && globalRain.game.IsMoonActive() && !globalRain.game.GetStorySession.saveState.deathPersistentSaveData.altEnding)
			{
				deathRainMode = DeathRainMode.GradeAPlateu;
	...
	public void Update()
	{
		...
		if (deathRain != null)
		{
			...
			bool flag = ((!ModManager.PrecycleModule) ? ((int)deathRain.deathRainMode > (int)DeathRain.DeathRainMode.GradeABuildUp) : (!forceSlowFlood));
			...
			else if ((ModManager.PrecycleModule || game.IsArenaSession) && (int)deathRain.deathRainMode >= (int)DeathRain.DeathRainMode.GradeABuildUp)
			{
				floodSpeed = Mathf.Min(1.8f, floodSpeed + 1f / 150f);
			}
			flood += floodSpeed;
			if (ModManager.PrecycleModule && Mathf.Max(game.cameras[0].room.world.RoomToWorldPos(game.cameras[0].pos, game.cameras[0].room.abstractRoom.index).y - 500f, game.cameras[0].room.world.RoomToWorldPos(new Vector2(0f, 0f), game.cameras[0].room.abstractRoom.index).y) <= flood)
			{
				forceSlowFlood = true;
			}
			return;
		...
		float num3 = room.roomSettings.GetEffectAmount(RoomSettings.RoomEffect.Type.HeavyRain);
		float num4 = room.roomSettings.GetEffectAmount(RoomSettings.RoomEffect.Type.HeavyRainFlux);
		float num5 = room.roomSettings.GetEffectAmount(RoomSettings.RoomEffect.Type.BulletRain);
		float effectAmount = room.roomSettings.GetEffectAmount(RoomSettings.RoomEffect.Type.BulletRainFlux);
		if (ModManager.Watcher)
		{
			if (global::Watcher.Watcher.cfgReducePassiveRainIntensity.Value < 0.5f)
			{
				num2 *= 1f - global::Watcher.Watcher.cfgReducePassiveRainIntensity.Value;
				num3 *= 1f - Mathf.Lerp(0f, 0.99f, global::Watcher.Watcher.cfgReducePassiveRainIntensity.Value * 2f);
				num5 *= 1f - Mathf.Lerp(0f, 0.99f, global::Watcher.Watcher.cfgReducePassiveRainIntensity.Value * 2f);
			}
			else
			{
				num5 = Mathf.Min(num5, 0.01f);
				num2 = Mathf.Max(num3, num2) * Mathf.Lerp(1f, 0f, (global::Watcher.Watcher.cfgReducePassiveRainIntensity.Value - 0.5f) * 2f);
				num3 = 0f;
			}
		}
		if (ModManager.PrecycleModule && preCycleRainPulse_Scale != 0f)
		{
			float num6 = num2;
			float num7 = num4;
			float num8 = num5;
			num2 = (1f + Mathf.Clamp(preCycleRainPulse_Intensity, 0f, 1f)) * preCycleRainPulse_Scale;
			num4 = Mathf.Clamp(num2 - 0.9f, 0f, 0.8f);
			if (room.roomSettings.DangerType == RoomRain.DangerType.Rain)
			{
				num5 = Mathf.Clamp(num2 - 0.9f, 0f, 1f);
			}
			if (num4 > 0.4f)
			{
				num5 = 0f;
		...
		if (num4 > 0f)
		{
			float num10 = 1200f * num4;
			float num11 = 60f;
			heavyTimer = (heavyTimer + 1) % (int)(num11 * 2f + num10 * 2f);
			if ((float)heavyTimer < num11)
			{
				num9 *= (float)heavyTimer / num11;
			}
			else if ((float)heavyTimer >= num11 + num10 && (float)heavyTimer < num11 * 2f + num10)
			{
				num9 *= 1f - ((float)heavyTimer - (num10 + num11)) / num11;
			}
			else if ((float)heavyTimer >= num11 * 2f + num10)
			{
				num9 = 0f;
			}
		}
		if (num9 > 0f)
		{
			Intensity = (1f + num9 * 4f) * 0.24f;
			RumbleSound = num9 * 0.2f;
			ScreenShake = num9;
		}
		else if (num2 > 0f)
		{
		...
		if (effectAmount > 0f)
		{
			float num13 = 1200f * effectAmount;
			float num14 = 60f;
			bulletTimer = (bulletTimer + 1) % (int)(num14 * 2f + num13 * 2f);
			if ((float)bulletTimer < num14)
			{
				num12 *= (float)bulletTimer / num14;
			}
			else if ((float)bulletTimer >= num14 + num13 && (float)bulletTimer < num14 * 2f + num13)
			{
				num12 *= 1f - ((float)bulletTimer - (num13 + num14)) / num14;
			}
			else if ((float)bulletTimer >= num14 * 2f + num13)
			{
				num12 = 0f;
			}
		}
	...
	public void ResetRain()
	{
		...
		if (game.cameras[0].hud != null && game.cameras[0].hud.rainMeter != null)
		{
