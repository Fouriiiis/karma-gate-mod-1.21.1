...
internal class WormGrass : UpdatableAndDeletable, IAccessibilityModifier, Explosion.IReactToExplosions
{
	public class Worm : UpdatableAndDeletable, IDrawable
	{
		...
		public Worm(WormGrass wormGrass, WormGrassPatch patch, Vector2 basePos, float reachHeight, float iFac, float lengthFac, bool cosmeticOnly, System.Random rnd)
		{
		...
		public override void Update(bool eu)
		{
			...
			if (attachedChunk == null && !Custom.DistLess(pos, basePos, length))
			{
				...
				Vector2 vector2 = vector * ((num - length) * 0.1f);
				pos += vector2;
				vel += vector2;
			}
			if (!culled)
			{
				...
				else
				{
					...
					for (int i = 0; i < segments.Length; i++)
					{
						...
						if (i > 0)
						{
							Vector2 vector3 = Custom.DirVec(segments[i].pos, segments[i - 1].pos);
							float num4 = Vector2.Distance(segments[i].pos, segments[i - 1].pos);
							Vector2 vector4 = vector3 * ((num4 - num2) * 0.25f);
							segments[i].pos += vector4;
							segments[i].vel += vector4;
							segments[i - 1].pos -= vector4;
							segments[i - 1].vel -= vector4;
							float a = num2 / num4;
		...
		public void DrawSprites(RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, float timeStacker, Vector2 camPos)
		{
			...
			else
			{
				...
				for (int i = 0; i < segments.Length; i++)
				{
					...
					Vector2 vector4 = vector3 * ((num + segments[i].stretchedRad) * 0.5f);
					(sLeaser.sprites[0] as TriangleMesh).MoveVertice(i * 4, vector - vector4 + normalized * num2 - camPos);
					(sLeaser.sprites[0] as TriangleMesh).MoveVertice(i * 4 + 1, vector + vector4 + normalized * num2 - camPos);
					if (i < segments.Length - 1)
					{
						Vector2 vector5 = vector3 * segments[i].stretchedRad;
						(sLeaser.sprites[0] as TriangleMesh).MoveVertice(i * 4 + 2, vector2 - vector5 - normalized * num2 - camPos);
						(sLeaser.sprites[0] as TriangleMesh).MoveVertice(i * 4 + 3, vector2 + vector5 - normalized * num2 - camPos);
					}
					else
					{
	...
	public class WormGrassPatch
	{
		...
		public WormGrassPatch(WormGrass wormGrass, IntVector2 firstTile)
		{
			...
			debugColor = new HSLColor((float)random.NextDouble(), 1f, 0.5f);
		...
		public void Update()
		{
			if (wormGrass.room.VoidWeaverActive)
			{
				pacified = Mathf.Max(pacified, 200);
			}
			if (pacified > 0)
			{
				pacified--;
			...
			else if (wormGrass.room.abstractRoom.creatures.Count > 0)
			{
				...
				if (realizedCreature != null && realizedCreature.abstractCreature.IsSameRippleLayer(0) && tiles.Count > 0 && realizedCreature.bodyChunks.Length != 0 && wormGrass.room.VisualContact(wormGrass.room.MiddleOfTile(tiles[UnityEngine.Random.Range(0, tiles.Count)]), realizedCreature.bodyChunks[UnityEngine.Random.Range(0, realizedCreature.bodyChunks.Length)].pos))
				{
					bool flag = ModManager.DLCShared && realizedCreature.WormGrassGooduckyImmune;
		...
		public void SortTiles()
		{
			...
			System.Random random = new System.Random(tiles[0].x + tiles[0].y + wormGrass.room.abstractRoom.index);
			...
			for (int k = 0; k < tiles.Count; k++)
			{
				...
				for (int l = 0; l < num2; l++)
				{
					...
					Vector2 vector = wormGrass.room.MiddleOfTile(tiles[k]) + new Vector2(-10f + 20f * num3 + Mathf.Lerp(-4f, 4f, (float)random.NextDouble()), -10f);
					float num4 = Mathf.Lerp(num, Mathf.Lerp(a, b, num3), 0.5f);
					float num5 = Mathf.Lerp(0.2f, 1f, Mathf.Pow((float)random.NextDouble(), 0.8f));
					if ((float)random.NextDouble() < sizes[k, 1])
					{
						list.Add(vector);
						list2.Add(num4);
						list3.Add(num5);
					...
					{
						Worm worm = new Worm(wormGrass, this, vector, num4, sizes[k, 1], num5, cosmeticOnly: false, random);
						worms.Add(worm);
		...
		for (int j = 0; j < patches.Count; j++)
		{
			for (int k = 0; k < patches[j].tiles.Count; k++)
			{
				...
				for (int l = 0; l < patches[j].cosmeticWormPositions[k].Length; l++)
				{
					...
					{
						Worm worm2 = new Worm(this, patches[j], patches[j].cosmeticWormPositions[k][l], patches[j].cosmeticWormLengths[k][l, 0], patches[j].sizes[k, 1], patches[j].cosmeticWormLengths[k][l, 1], cosmeticOnly: true, rnd);
