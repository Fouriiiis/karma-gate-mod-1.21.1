...
/**
 * Controller entity for a Small Centipede.
 * C# SmallCentipede: size = 0, 5 segments, edible, can shock when grabbed.
 * Only hunts chickens.
 */
public class SmallCentipedeEntity extends HostileEntity implements GeoAnimatable, CentipedeController {
    ...
    private static final TrackedData<Boolean> BODY_DIRECTION = DataTracker.registerData(
            SmallCentipedeEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    private static final TrackedData<Float> SHOCK_CHARGE = DataTracker.registerData(
            SmallCentipedeEntity.class, TrackedDataHandlerRegistry.FLOAT);
    ...
    // --- Small centipede shell colors ---
    // C# small centipede: orange-ish like normal but at size=0
    // HSL(0.04, 0.9, 0.5) ≈ RGB(242, 49, 13)
    private static final int SHELL_COLOR = (242 << 16) | (49 << 8) | 13;
    // HSL(0.04, 0.9, 0.3) ≈ RGB(145, 30, 8)
    private static final int SECONDARY_SHELL_COLOR = (145 << 16) | (30 << 8) | 8;

    public SmallCentipedeEntity(EntityType<? extends HostileEntity> type, World world) {
        super(type, world);
        this.noClip = true;
        this.setNoGravity(true);
        java.util.Arrays.fill(segmentIds, -1);
        // Use red centipede's MAX_RADIUS (8.0) as the reference denominator so that
        // scaleFactor = computeRadius(i) / 8.0 gives ~0.5 at the middle segment,
        // producing a creature half the visual size of the red centipede.
        this.maxRadius = 8.0f;
    }
    ...
    protected void initGoals() {
        ...
        // SmallCentipedes only hunt chickens
    ...
    private void spawnSegments() {
        ...
        for (int i = 0; i < TOTAL_SEGMENTS; i++) {
            ...
            // C#: Small centipedes have no shells
    ...
    /**
     * Compute segment radius at size=0.
     * C# formula with size=0: minR=2, maxR=4, pow exponent=0.7
     */
    @Override
    ...
    public float getHeadScaleFactor() {
        // Heads scale down proportionally: smallCenti head radius (~2) / ref max (8) = 0.25,
        // but we use 0.5 to keep them visually in proportion with the halved body.
        return 0.5f;
    }
    ...
    private void updateChainPhysics() {
        ...
        // Crawl propulsion — size=0 so speed is at minimum
        if (moving) {
            for (int i = 1; i < TOTAL_SEGMENTS - 1; i++) {
                ...
                if (ahead >= 0 && ahead < TOTAL_SEGMENTS && segments[ahead] != null && !segments[ahead].isRemoved()) {
                    if (isNearSurface(segments[ahead])) {
                        ...
                        // size=0: speed mult = lerp(0, 0.5, 1.5) = 0.5
        ...
        // Stiffness — size=0 so stiffness is at minimum
    ...
    private boolean isValidPrey(LivingEntity entity) {
        ...
        // SmallCentipedes only prey on chickens
    ...
    private void updateShockCharge() {
        ...
        if (head0Grab && head1Grab) {
            ...
            if (sameTarget || headsCloseToTarget) {
                // C# Small: charge rate = 1/Lerp(100,5,0) = 1/100
    ...
    private void shock(LivingEntity victim) {
        ...
        // C# Small centipede shock: weak, just stuns
